...
export default function DebotScreen() {
  const [file, setFile] = useState<File | null>(null);
  const [busy, setBusy] = useState(false);
  const [wiping, setWiping] = useState(false);
  const [downloading, setDownloading] = useState(false);
  const [result, setResult] = useState<Awaited<ReturnType<typeof dummyImport>> | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const dummyImport = async (_file: File) => {
    ...
    return {
      ...
      partiesCreated: 5,
      itemsCreated: 8,
      returns: { totalInFile: 3, imported: 3, skippedExisting: 0 },
      journals: { totalInFile: 7, imported: 6, skippedExisting: 1, skippedNoParty: 0 },
      masters: {
        accountsInFile: 12,
        partiesCreated: 5,
        partiesSkippedExisting: 6,
        partiesSkippedNonParty: 1,
        itemsInFile: 15,
        itemsCreated: 8,
        itemsSkippedExisting: 7,
      },
      errors: [] as string[],
      skipped: [] as string[],
    };
  ...
  const handleImport = async () => {
    if (!file) return toast.error("Choose a .DEB file");
    setBusy(true);
    setResult(null);
    try {
      const r = await dummyImport(file);
      setResult(r);
      const bits: string[] = [];
      ...
      if (r.masters.accountsInFile > 0) bits.push(`${r.partiesCreated} parties`);
      if (r.masters.itemsInFile > 0) bits.push(`${r.itemsCreated} items`);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-3xl">
      <header>
        <h1 className="font-display text-3xl md:text-4xl tracking-wide">Debot Import</h1>
        <p className="text-sm text-muted-foreground mt-1">One-way import from Debot Accounting exports</p>
      </header>
      ...
      <div className="premium-card rounded-2xl p-8 marble-noise relative">
        ...
        >
          ...
          <p className="font-display text-xl mt-3">{file ? file.name : "Drop DEB file or click to browse"}</p>
          <p className="text-xs text-muted-foreground mt-1">Accepts .DEB (Debot export format)</p>
          <input ref={inputRef} type="file" accept=".deb,.xml" hidden onChange={(e) => setFile(e.target.files?.[0] || null)} />
        </div>
        ...
        <div className="mt-6 text-xs text-muted-foreground bg-muted/30 rounded-lg p-4 space-y-1">
          ...
          <p>• Upload Debot export files to bring in invoices, parties, items, and journals</p>
          <p>• Cash invoices auto-record full payment, credit invoices stay outstanding</p>
          <p>• Idempotent — existing records are <strong>skipped</strong>, safe to re-upload</p>
          <p>• Import the masters file first, then the voucher file for best results</p>
        </div>
      </div>
      ...
      <div className="premium-card rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <button
            onClick={async () => {
              setDownloading(true);
              try { toast.success("Backup downloaded"); }
        ...
        <div className="premium-card rounded-2xl p-6 space-y-4">
          ...
          {result.skipped && result.skipped.length > 0 && (
            <div className="bg-muted/30 border border-border rounded-lg p-4 text-xs space-y-1 max-h-64 overflow-y-auto">
              <p className="font-medium text-muted-foreground">{result.skipped.length} non-standard vouchers skipped (safe to ignore)</p>
